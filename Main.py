...
def main_loop(emitter, initial_settings):
    ...
    model_path = os.path.join(script_dir, "..", "runs", "detect", "mahjong_nano_v3", "weights", "best.pt")
    ...
    while True:
        ...
        # Only run deep shanten logic and calculate discards if we have exactly 14 tiles (our turn to discard)
        if tile_count == 14:
            ...
            # Map Ukeire data onto the hand
            ukeire_map = dict(best_discards)
            for item in hand_data:
                # Need to map the filename back to index... 
                # we can just build this dynamically during the iteration instead to avoid reverse lookups.
                pass 
            
            # Rebuild hand mapping properly
            hand_data = []
            for index, count in enumerate(state.hand_array):
                for _ in range(count):
            ...
            discard_filenames = [GameState.TILE_INDEX_TO_ASSET[idx[0]] for idx in best_discards[:3]]
            
            emitter.update_ui_signal.emit({
                "status": "Locked (Turn active)",
                "shanten": current_shanten,
                "hand_data": hand_data,
                "discards": discard_filenames,
